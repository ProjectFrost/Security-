...
- Jump server
	Access secure network zones. Hardened and monitored. SSH/TUNNEL/VPN to the jump server
- HSM → Like TPM but bigger for enterprises. Also cryptographic accelerators
- Sensors and collectors → Aggregate information from network devices


# Wireless Cryptography

## WPA2
- Uses CCMP block cipher mode. Data encryption with AES. Message Integrity Check with CBC-MAC
- Has a PSK brute-force problem → Capture the PSK hash. Some methods can derive the PSK hash without the handshake.
## WPA3
- Uses Galois/Counter Mode Protocol. Data encryption with AES. Message Integrity Check with GMAC
- Includes mutual authentication. Creates a shared session key without sending that key across the network. Adds perfect forward secrecy
- SAE (Simultaneous Authentication of Equals) → Everyone uses a different session key
...
- Configuration part of the wireless network connection
## Modes 
- WPA3-Personal → PSK same for everyone. Unique session keys using SAE
- WPA3-Enterprise → Uses 802.1X to authenticate every user individually
## Captive portal
Access table recognizes a lack of authentication and redirects your web access to a captive portal page
## WPS
Allows “easy” setup of a mobile device. PIN, Push a button on the access point. NFC. PIN can be brute-forced. Better to disable WPS

## Protocols
- EAP(Extensible Authentication Protocol)
	Integrates with 802.1X
- EAP-FAST(EAP Flexible Authentication via Secure Tunneling)
	Authentication server and supplicant share a protected access credential(PAC)
...
- PEAP(Protected Extensible Authentication Protocol)
	Encapsulates EAP in a TLS tunnel. Uses digital certificate instead of  PAC, client doesnt need it.
	Usually combined with MSCHAPv2 aka Microsoft’s Challenge which the user uses for authentication
	User can also authenticate with a GTC(Generic Token Card)
- EAP-TLS
	Strong security but requires both client and server to have digital certificates
- EAP-TTLS(EAP-Tunneled TLS)
	Support other authentication protocols in a TLS tunnel. Requires a digital certificate on the AS. 
	User uses whatever the fuck they want as authentication method
- RADIUS Federations
	Uses 802.1X as the authentication method. RADIUS on backend, EAP to authenticate
